...
<div>
    <div class="p-3 bg-white border-t flex flex-row">
        ...
        <button class="p-2 text-white bg-blue-600 w-auto text-sm ml-4" @click="openModalAddJualBeli = true">Tambah Transaksi Baru</button>
    </div>
    <div class="p-2">
        <div>
                ...
                <tbody class="bg-white">
                    <tr class="even:bg-slate-200 even:text-black" v-for="(jb, index) in tabelJualBeli" :key="index">
                        ...
                        <td class="border border-white p-3 text-center">
                            ...
                            <router-link class="p-2 text-white bg-blue-800 w-auto text-sm ml-4" v-if="jb.status_transaksi == 'pending'" :to="{ name: 'JualBeliMurabahahDetail', query: { id: 1 }}">Verifikasi</router-link>
                        </td>
                    </tr>
                </tbody>
            </table>
                ...
                <div class="grid grid-rows-1">
                    ...
                    <div class="grid grid-rows-1 gap-2 mb-10">
                        ...
                        <select class="border border-slate-500 bg-white p-1" v-model="formJualBeli.kd_nasabah">
                            ...
                            <option v-for="(cf, index) in listCustomer" :key="index" :value="cf.id">{{ cf.kd_identitas }} - {{ cf.nama_sesuai_identitas }}</option>
                        </select>
                        <label class="font-bold text-black">Judul Permintaan</label>
                        <input type="text" class="border border-slate-500 bg-white p-1" v-model="formJualBeli.nama_permintaan" />
                        <label class="font-bold text-black">Deskripsi Permintaan</label>
                        <textarea class="border border-slate-500 bg-white p-1 h-[150px]" v-model="formJualBeli.deskripsi_permintaan"></textarea>
                        <label class="font-bold text-black">Link Dokumen Pendukung</label>
                        <input type="text" class="border border-slate-500 bg-white p-1" v-model="formJualBeli.link_pendukung" />
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <button class="bg-slate-300 text-black p-2 rounded-md" @click="openModalAddJualBeli = false">Tutup</button>
                        <button class="bg-blue-600 text-white p-2 rounded-md" @click="tambahJualBeli">Simpan</button>
                    </div>
                </div>
            </div>
        </div>
